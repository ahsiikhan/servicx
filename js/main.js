...
(()=>{
	...
	//attach an event handler to documnet
	document.addEventListener("click", (event)=>{
		if(event.target.classList.contains('link-item')){
			/*make sure event.target.hash has a value before overriding default behaviour*/
			if(event.target.hash !==""){
				//prevent default anchor click behavoiur
				event.preventDefault();
				const hash = event.target.hash;
				//deactivate esisting active 'section'
				...
				/* if clicked 'link-item' is contained withinn the 'navigation menu'*/

				if(navMenu.classList.contains("open")){
					//activate new navigation menu 'link-item'
					event.target.classList.add("active","inner-shadow");
					event.target.classList.remove("outer-shadow","hover-in-shadow");
					//hide navingation menu
					hideNavMenu();
...
(() =>{
	const filterContainer = document.querySelector(".portfolio-filter"), portfolioItemsContainer = document.querySelector(".portfolio-items"),
